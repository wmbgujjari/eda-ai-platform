...
from FG.services.revenue_db_service import fetch_revenue_monthly_data_sectionwise,fetch_revenue_weather_data
from fastapi import HTTPException
from pycaret.regression import setup, compare_models, pull, save_model, finalize_model
from FG.core.utils.FGautoML import get_regression_model
...
async def load_data(section_id: str,task_id: str,train_year: str,trian_month: str):
    try:
        ...
        df = await fetch_revenue_monthly_data_sectionwise(section_id,task_id,train_year,trian_month)
        print("loding df")
        df_weather = await fetch_revenue_weather_data(task_id,train_year,trian_month)
        print("loaiding df_weather")
        if df is None:
            raise Exception("fetch_revenue_collections_monthly_data() returned None")
        
        if df_weather is None:
            raise Exception("fetch_revenue_weather_data() returned None")
...
def process_data(df, df_weather):
    ...
    #df = df.rename(columns={"COLLECTION": "ACTUAL_CONSUMPTION"})
    ...
    df_weather["REV_MONTH"] = df_weather["DATE"].dt.month
    df_weather["REV_YEAR"] = df_weather["DATE"].dt.year
    monthly_weather = (
    ...
    df_merged = pd.merge(df, monthly_weather,
                         on=["REV_MONTH", "REV_YEAR"], how="left")

    # --- Feature Engineering ---
    df_merged["Section_Monthly_Avg"] = (
        df_merged.groupby(["SECTION_ID", "REV_MONTH"])["COLLECTION"]
        .transform("mean")
        .round(2)
    )
    df_merged["Consumption_vs_Avg"] = (
        df_merged["COLLECTION"] - df_merged["Section_Monthly_Avg"]
    )

    df_merged['REV_DATE'] = pd.to_datetime(dict(
        year=df_merged['REV_YEAR'],
        month=df_merged['REV_MONTH'],
        day=1
    ))
    ...
    df_merged['Previous_month_Consumption'] = (
        df_merged.groupby('SECTION_ID')["COLLECTION"]
        .shift(1).fillna(0)
    )

    # Sort
    df_sorted = df_merged.sort_values(['SECTION_ID', 'REV_DATE'])
