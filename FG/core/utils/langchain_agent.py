...
@shared_task
def save_reasoning_log_to_db(model_name: str, division_id: str, task_id: str):
    """
    ...
    try:
        ...
        logging.info(f"[LangChain] Summary stored for model={model_name}, division={division_id}")
    except Exception as e:
    ...
    try:
        ...
        insert_sql = """
            INSERT INTO MIS_USER.MODEL_REASONING_LOGS (
                MODEL_NAME, DIVISION_ID, TASK_ID, SUMMARY, TIMESTAMP
            )
            VALUES (:1, :2, :3, :4, TO_DATE(:5, 'YYYY-MM-DD HH24:MI:SS'))
        """

        cursor.execute(insert_sql, (model_name, division_id, task_id, summary, timestamp))
        conn.commit()
