...
from FG.services.train_revenue import train_with_pandas,load_data,train_with_automl
...
@shared_task(name="FG.tasks.revenue_tasks.schedule_train_revenue")
def schedule_train_revenue(train_year: str, train_month: str, office_id: str,model_name: str):
    logger.info("✅ Beat triggered schedule_train_revenue_tasks")
    ...
    url = f"http://127.0.0.1:8000/revenue/train/{train_year}/{train_month}/{office_id}/{model_name}"
...
def select_or_increment_model(df, df_weather, task_id, model_name: str):
    try:
        ...
        if model_name == "AutoML":
            ...
            # Train AutoML model (implement your AutoML training here)
        ...
        last_year = train_df["REV_YEAR"].max()
        last_month = train_df.loc[train_df["REV_YEAR"] == last_year, "REV_MONTH"].max()
        ...
        #    train_df, df_weather_future, future_months, 
        #    features if trained_with == "pandas" else None, 
        #    model
        #)
        if trained_with == "automl":
            print("automl generate_future_prediction_input_avghistory_automl")
            future_df = generate_future_prediction_input_avghistory_automl(
            train_df, df_weather_future, future_months,features, model,le_section_id
            )
        else:
            future_df = generate_future_prediction_input_avghistory(
            train_df, df_weather_future, future_months, features, model
        )
        ...
        section_data_map = {
            section_id: [
                {
                    "REV_MONTH": int(row["REV_MONTH"]) if pd.notnull(row["REV_MONTH"]) else None,
                    "REV_YEAR": int(row["REV_YEAR"]) if pd.notnull(row["REV_YEAR"]) else None,
                    "OFFICE_ID": str(row["SECTION_ID"]).strip() if pd.notnull(row["SECTION_ID"]) else None,
                    "COLLECTION": safe_float(row.get("COLLECTION", 0)),
                    "Predicted_Collection": safe_float(row.get("PREDICTED_VALUE", 0)),
...
def generate_future_prediction_input_avghistory(
    ...
    rolling_window: number of months for autoregressive rolling mean
    alpha: weight for autoregressive component
    """
    import numpy as np
    import pandas as pd
    import logging
    ...
    # Precompute section monthly averages
    section_month_avg = (
        train_df.groupby(["SECTION_ID", "REV_MONTH"])["COLLECTION"]
        .mean()
        .to_dict()
    )
    ...
    # Pre-aggregate weather monthly
    monthly_weather = (
        df_weather.groupby(["REV_YEAR", "REV_MONTH"]).mean(numeric_only=True).reset_index()
    )
    ...
    for dt in future_months:
        ...
        # Get monthly weather
        month_weather = monthly_weather[
            (monthly_weather["REV_YEAR"] == year) & (monthly_weather["REV_MONTH"] == month)
        ]
        if month_weather.empty:
            month_weather = pd.DataFrame([{col: 0 for col in weather_cols + ["REV_YEAR", "REV_MONTH"]}])
        ...
        for sec in train_df["SECTION_ID"].unique():
            ...
            if train_df[actual_available].shape[0] > 0:
                row["COLLECTION"] = train_df.loc[actual_available, "COLLECTION"].values[0]
            else:
                row["COLLECTION"] = None

            # Previous consumption: rolling autoregressive mean
            if rolling_prev[sec]:
                row["Previous_month_Consumption"] = np.mean(rolling_prev[sec][-rolling_window:])
            else:
                row["Previous_month_Consumption"] = 0
...
@shared_task
def insert_accuracy_log(section_id, model_version, training_date, trained_with, r2, rmse,dashboard,task_id: str):
    conn = None
    cursor = None
    try:
        ...
        cursor.execute(insert_sql, (section_id, model_version, training_date, trained_with, r2, rmse,dashboard))
        conn.commit()
...
def insert_section_data(section_id: str, records: List[dict], task_id: str, model_name: str):
    ...
    try:
        ...
        if records and "MODEL_NAME" in records[0]:
            model_name_full = str(records[0]["MODEL_NAME"]).strip()
        else:
            model_name_full = model_name    
        # ✅ Step 1: Delete previous prediction records for this section
        delete_sql = """
            DELETE FROM MIS_USER.REVENUE_PREDICTION
            WHERE office_id = :office_id AND model = :model_name
        """
        cursor.execute(delete_sql, {"office_id": section_id,"model_name": model_name_full})
        conn.commit()
        ...
        insert_sql = """
            INSERT INTO MIS_USER.REVENUE_PREDICTION (
                rev_month, rev_year, office_id, actual, predicted, model, record_status
            )
        ...
        data_tuples = [
            (
                int(rec["REV_MONTH"]) if rec.get("REV_MONTH") else None,
                int(rec["REV_YEAR"]) if rec.get("REV_YEAR") else None,
                str(rec["OFFICE_ID"]).strip() if rec.get("OFFICE_ID") else None,
                safe_float(rec.get("COLLECTION")),
                safe_float(rec.get("Predicted_Collection")),
