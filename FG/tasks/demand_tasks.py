...
from FG.services.train_demand import train_with_pandas,load_data,train_with_automl
...
from FG.core.database_uat import get_uat_db_connection, uat_session_pool
logger = logging.getLogger(__name__)


celery = Celery(
    "FG.tasks.demand_tasks",
...
@shared_task(name="FG.tasks.demand_tasks.schedule_train_demand")
def schedule_train_demand(train_year: str, train_month: str, office_id: str,model_name: str):
    ...
    url = f"http://127.0.0.1:8000/demand/train/{train_year}/{train_month}/{office_id}/{model_name}"
...
def select_or_increment_model(df, df_weather, task_id, model_name: str):
    try:
        ...
        if trained_with == "automl":
            ...
            target_col = model_bundle.get("target_col", "ACTUAL_CONSUMPTION")
        ...
        section_accuracy_map[section_id] = {
            "MODEL_VERSION": f"Demand_{model_name}_{train_df['BILLMONTH'].max()}_{train_df['BILLYEAR'].max()}",
            ...
            section_id: [
                {
                    ...
                    "MODEL_NAME": "automl_model" if trained_with == "automl" else f"Demand_{model_name}_{train_df['BILLMONTH'].max()}_{train_df['BILLYEAR'].max()}",
                    "VERSION": "1",
                }
...
@shared_task
def insert_accuracy_log(section_id, model_version, training_date, trained_with, r2, rmse,dashboard: str, task_id: str):
    conn = None
    cursor = None
    try:
